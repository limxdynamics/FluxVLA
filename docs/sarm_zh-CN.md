...
这些文件现在作为 starter template 提供，默认 `data_root_path` 使用通用示范路径 `./datasets/your_sarm_lerobot_dataset`，不会再默认指向 libero 数据。使用前请改成你自己的数据集路径；如果相机键不是 `observation.images.image`，也一并修改或通过 `--cfg-options` 覆盖。
