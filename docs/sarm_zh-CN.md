...
这些文件现在默认指向已经发布到 Hugging Face 的 manual SARM 示例数据集 `./datasets/SARM_manual_test_10Episodes_lerobotv3.0`，并使用与该数据集匹配的 `observation.images.cam_high` 作为默认相机键。新的 `./datasets/SARM_manual_test_10Episodes_lerobotv2.1` 转换版也已经在这三套 SARM 配置上完成了端到端 smoke test，只需通过 `--cfg-options` 覆盖数据根路径即可。如果你把数据下载到了其他路径，或者想切换到其他相机流，可以通过 `--cfg-options` 覆盖 `data_root_path` 与 `video_keys`。

已发布的 Hugging Face 数据集：

- LeRobot v3.x 版、训练 / 推理用的完整人工标注数据：[`limxdynamics/FluxVLAData/SARM_manual_test_10Episodes_lerobotv3.0`](https://huggingface.co/datasets/limxdynamics/FluxVLAData/tree/main/SARM_manual_test_10Episodes_lerobotv3.0)
