...
def ruckig_filter(
    ...
    Args:
        ...
        max_settle_steps: Maximum extra steps after the last target to
            ...
            0 (default) means no extra steps.
        resample_settle: If True (default), resample extra settle points
            back to ``n_targets`` length.  If False, keep all settle
            steps at their original resolution.
    """
    if Ruckig is None:
        raise ImportError(
            'ruckig package is required. Install with: pip install ruckig')
    ...
    inp.max_velocity = broadcast_limits(max_velocity, n_dof)
    inp.max_acceleration = broadcast_limits(max_acceleration, n_dof)
    inp.max_jerk = broadcast_limits(max_jerk, n_dof)

    # positions[0] = init state
    pos_list = [np.array(inp.current_position)]
    vel_list = [np.array(inp.current_velocity)]
    acc_list = [np.array(inp.current_acceleration)]
    ...
    # Resample extra settle points back to original length.
    # The settle trajectory covers (n_total-1)·dt seconds; the output must
    # have n_targets points at dt spacing.  Resampling therefore compacts
    # time by factor c, which scales v by c and a by c².
    if resample_settle and len(positions) > n_targets:
        n_total = len(positions)
        c = (n_total - 1) / max(n_targets - 1, 1)
        src_idx = np.linspace(0, n_total - 1, n_targets)
        lo = np.floor(src_idx).astype(int)
        hi = np.minimum(lo + 1, n_total - 1)
        alpha = (src_idx - lo)[:, np.newaxis]
        positions = positions[lo] + alpha * (positions[hi] - positions[lo])
        velocities = c * (
            velocities[lo] + alpha * (velocities[hi] - velocities[lo]))
        accelerations = c * c * (
