    ...
    def __init__(
        ...
        for d_name, d_weight in data_mix:
            if d_name in included_datasets:
                ...
                print(
                    f'Skipping Duplicate Dataset: `{(d_name, d_weight)}`'  # noqa: W604,E501
                )
                continue
        ...
        for d_name, d_weight in filtered_mixture_spec:
            try:
                ...
                # TODO: Change to logging
                print(f'Skipping `{d_name}` due to Error: {e}')  # noqa: W604
    ...
    def __call__(self, inputs: Dict[str, Any]) -> Dict[str, Any]:
        ...
        for img_key in self.img_keys:
            if img_key not in inputs:
                raise KeyError(
                    f'Image key `{img_key}` not found in inputs!'  # noqa: W604,E713,E501
                )
        ...
        if isinstance(image_out, tuple):
            ...
            image_grid_thw = None
        # Build VLA prompt
        prompt = f'In: What action should the robot take to {task_description.lower()}?\nOut:' + self.prompt_suffix  # noqa: E231,E501
