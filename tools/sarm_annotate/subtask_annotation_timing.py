...
try:  # when executed as part of a package
    from .subtask_annotation import (
        ...
        generate_auto_sparse_annotations,
...
def resolve_dataset_spec(dataset_source: str) -> tuple[str, Path | None]:
    ...
    if source_path.exists():
        ...
        if not info_path.exists():
            raise ValueError(f"Local dataset path does not look like a LeRobot dataset: {source_path}")
...
def compute_episode_duration_s(dataset_meta: Any, episode_index: int, video_key: str) -> float:
    start = float(dataset_meta.episodes[f"videos/{video_key}/from_timestamp"][episode_index])
    end = float(dataset_meta.episodes[f"videos/{video_key}/to_timestamp"][episode_index])
    return end - start


def compute_weighted_average(records: list[dict[str, Any]], field_name: str) -> float | None:
    weighted_sum = 0.0
    total_weight = 0.0
...
def compute_unweighted_average(records: list[dict[str, Any]], field_name: str) -> float | None:
...
def save_proportions(
    ...
    props: dict[str, float] = {"task": 1.0} if is_auto else compute_temporal_proportions(annotations, fps, subtask_list)
...
def format_timing_line(record: dict[str, Any]) -> str:
    pieces = [
        ...
        f"duration={record['episode_duration_s']:.3f}s" if record["episode_duration_s"] is not None else None,
...
def save_timing_summary(
    ...
    sorted_records = sorted(timing_records, key=lambda item: item["episode_index"])
    successful_records = [record for record in sorted_records if record["total_vlm_time_s"] > 0]
    ...
    summary = {
        ...
        "num_timed_episodes": len(sorted_records),
        "num_successful_timed_episodes": len(successful_records),
        "average_total_vlm_time_s": compute_weighted_average(successful_records, "total_vlm_time_s"),
        "average_sparse_vlm_time_s": compute_weighted_average(sorted_records, "sparse_vlm_time_s"),
        "average_dense_vlm_time_s": compute_weighted_average(sorted_records, "dense_vlm_time_s"),
        "unweighted_average_total_vlm_time_s": compute_unweighted_average(successful_records, "total_vlm_time_s"),
        "unweighted_average_sparse_vlm_time_s": compute_unweighted_average(sorted_records, "sparse_vlm_time_s"),
        "unweighted_average_dense_vlm_time_s": compute_unweighted_average(sorted_records, "dense_vlm_time_s"),
        "episodes": sorted_records,
    }
    ...
    if summary["average_total_vlm_time_s"] is not None:
        print(f"Weighted average total VLM time per episode: {summary['average_total_vlm_time_s']:.3f}s")
    if summary["average_sparse_vlm_time_s"] is not None:
        print(f"Weighted average sparse VLM time per episode: {summary['average_sparse_vlm_time_s']:.3f}s")
    if summary["average_dense_vlm_time_s"] is not None:
        print(f"Weighted average dense VLM time per episode: {summary['average_dense_vlm_time_s']:.3f}s")
...
def worker_process_episodes_timed(
    ...
    sparse_annotator = (
        VideoAnnotator(sparse_subtask_list, model_name, device, torch_dtype) if sparse_subtask_list else None
    )
    dense_annotator = (
        VideoAnnotator(
            dense_subtask_list,
    ...
    dense_annotations: dict[int, Any] | None = {} if dense_subtask_list else None
    timing_records: list[dict[str, Any]] = []

    for ep_idx in episode_indices:
        record = build_timing_record(ep_idx)
        record["episode_duration_s"] = compute_episode_duration_s(dataset.meta, ep_idx, video_key)
...
def main() -> None:
    parser = argparse.ArgumentParser(description="SARM subtask annotation with per-episode VLM timing")
    parser.add_argument("--repo-id", type=str, required=True, help="Dataset repo id or full local dataset path")
    parser.add_argument(
    ...
    parser.add_argument(
        ...
        type=int,
        default=None,
        help="Randomly sample this many episodes from the candidate set before annotation",
    )
    parser.add_argument(
        "--random-seed",
        type=int,
    ...
    parser.add_argument(
        ...
        type=str,
        default=None,
        help="Optional JSON output path for timing summary (default: <output-dir>/vlm_timing_summary.json)",
    )
    ...
    if args.dense_subtasks and not args.sparse_subtasks and not args.dense_only:
        print("Error: --dense-subtasks requires --sparse-subtasks or --dense-only")
        return

    sparse_subtask_list = (
        [item.strip() for item in args.sparse_subtasks.split(",")] if args.sparse_subtasks else None
    )
    dense_subtask_list = [item.strip() for item in args.dense_subtasks.split(",")] if args.dense_subtasks else None
    ...
    if args.skip_existing:
        episode_indices = [episode for episode in episode_indices if episode not in existing_annotations]
    ...
    if need_vlm:
        if args.num_workers > 1:
            ...
            for index, episode_idx in enumerate(episode_indices):
                episodes_per_worker[index % args.num_workers].append(episode_idx)
            ...
            for index, episode_idx in enumerate(episode_indices):
                print(f"Episode {episode_idx} ({index + 1}/{len(episode_indices)})")
                record = build_timing_record(episode_idx)
                record["episode_duration_s"] = compute_episode_duration_s(dataset.meta, episode_idx, video_key)
                record["timing_weight"] = record["episode_duration_s"]

                if sparse_annotator:
                    start_time = time.perf_counter()
                    _, sparse_ann, sparse_err = process_single_episode(
                        episode_idx, dataset.root, dataset.meta, video_key, fps, sparse_annotator
                    )
                    record["sparse_vlm_time_s"] = time.perf_counter() - start_time
                    if sparse_ann:
                ...
                if dense_annotator:
                    start_time = time.perf_counter()
                    _, dense_ann, dense_err = process_single_episode(
                        episode_idx, dataset.root, dataset.meta, video_key, fps, dense_annotator
                    )
                    record["dense_vlm_time_s"] = time.perf_counter() - start_time
                    if dense_ann and dense_annotations is not None:
    ...
    save_proportions(dataset.root, sparse_annotations, fps, "sparse", sparse_subtask_list, auto_sparse)
    if dense_mode and dense_annotations:
        save_proportions(dataset.root, dense_annotations, fps, "dense", dense_subtask_list)

    print(f"\nComplete! {len(sparse_annotations)} sparse, {len(dense_annotations or {})} dense annotations")

    if need_vlm:
        timing_output = Path(args.timing_output) if args.timing_output else Path(args.output_dir) / "vlm_timing_summary.json"
        save_timing_summary(
