...
def _resolve_block(
    ...
    if not isinstance(value, dict):
        raise ValueError(f"Unsupported stage block type: {type(value).__name__}")
    ...
    if not (len(names) == len(starts) == len(ends)):
        raise ValueError(
            f"Stage block lengths disagree: names={len(names)}, "
    ...
    return {
        ...
        "start_times": (
            [float(t) for t in value["start_times"]]
            if value.get("start_times") is not None
            else None
        ),
        "end_times": (
            [float(t) for t in value["end_times"]]
            if value.get("end_times") is not None
            else None
        ),
    }
...
def _accumulate_proportions(
    ...
    for name, start, end in zip(
        block["names"], block["start_frames"], block["end_frames"]
    ):
...
def _apply_block_v3(df: pd.DataFrame, col_prefix: str,
    ...
    if any(b and b.get("start_times") is not None for b in blocks):
        df[start_t_col] = [
            b["start_times"] if (b and b.get("start_times") is not None) else None
            for b in blocks
        ]
...
def _rewrite_parquet(
    ...
    for path in parquet_files:
        ...
        for _, row in df.iterrows():
            ...
            entry = spec.get(ep_idx, {})
            sparse_block = _resolve_block(entry.get("sparse", default_sparse), length)
...
def main() -> None:
    ...
    parser.add_argument("--default-sparse", default=None,
                        help="Fallback for episodes missing from spec. "
                             "Use 'auto' for a single 'task' stage.")
    parser.add_argument("--default-dense", default=None,
                        help="Fallback for episodes missing from spec. "
                             "Use 'auto' for a single 'task' stage.")
