...
def main() -> None:
    """Parse dense per-episode annotation metadata into JSON."""
    parser = argparse.ArgumentParser(
        description='Parse dense-only subtask info per episode.')
    parser.add_argument(
        '--dataset-root',
        type=Path,
        required=True,
        help=
        'Dataset root directory containing meta/episodes and meta/temporal_proportions_dense.json',
    )
    parser.add_argument(
        '--output',
        type=Path,
    ...
    for _, row in episodes_df.iterrows():
        ...
        temporal_props = _compute_temporal_proportions(dense_names,
                                                       dense_starts,
                                                       dense_ends)
