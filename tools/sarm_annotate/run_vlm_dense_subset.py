...
def worker_process_dense_episodes(
    ...
    for episode_index in episode_indices:
        ...
        record["episode_duration_s"] = compute_episode_duration_s(dataset.meta, episode_index, video_key)
        ...
        if dense_annotation is not None:
            dense_annotations[episode_index] = serialize_annotation(dense_annotation)
...
def main() -> None:
    parser = argparse.ArgumentParser(description="Run dense VLM annotation on a selected episode subset")
    parser.add_argument("--dataset-source", type=str, required=True, help="Dataset repo id or local dataset path")
    parser.add_argument("--episodes", type=int, nargs="+", required=True, help="Global episode indices to annotate")
    parser.add_argument("--dense-subtasks", type=str, required=True, help="Comma-separated dense subtasks")
    parser.add_argument("--video-key", type=str, required=True, help="Dataset video key")
    parser.add_argument("--model", type=str, required=True, help="VLM model path or name")
    parser.add_argument("--output-path", type=str, required=True, help="Path to save dense annotation JSON")
    parser.add_argument("--timing-output", type=str, required=True, help="Path to save timing summary JSON")
    parser.add_argument("--num-workers", type=int, default=1, help="Number of parallel VLM workers")
    parser.add_argument("--gpu-ids", type=int, nargs="+", default=None, help="Explicit GPU IDs to use")
    parser.add_argument("--device", type=str, default="cuda", help="Fallback device for single-worker mode")
    parser.add_argument(
        "--dtype",
        type=str,
        default="bfloat16",
    ...
    dense_subtask_list = [item.strip() for item in args.dense_subtasks.split(",") if item.strip()]
    ...
    dataset = LeRobotDataset(repo_id, root=dataset_root, download_videos=True, episodes=args.episodes)
    if args.video_key not in (dataset.meta.video_keys or []):
        raise ValueError(f"Video key '{args.video_key}' not found. Available: {dataset.meta.video_keys}")
    ...
    if args.num_workers > 1 and torch.cuda.is_available():
        gpu_ids = args.gpu_ids or list(range(min(args.num_workers, torch.cuda.device_count())))
        ...
        dense_annotator = VideoAnnotator(dense_subtask_list, args.model, device, torch_dtype)
        for episode_index in selected_episode_indices:
            record = build_timing_record(episode_index)
            record["episode_duration_s"] = compute_episode_duration_s(dataset.meta, episode_index, args.video_key)
            record["timing_weight"] = record["episode_duration_s"]
    ...
    missing_episode_indices = [
        episode_index for episode_index in selected_episode_indices if episode_index not in dense_annotations
    ]
    if missing_episode_indices:
        raise RuntimeError(f"Dense VLM annotation failed for episodes: {missing_episode_indices}")
    ...
    annotations_payload = {
        ...
        "annotations": {str(index): dense_annotations[index] for index in sorted(dense_annotations)},
    }
    with open(output_path, "w", encoding="utf-8") as file:
